...
{{--            @slot('title') Редактирование заказа № {{$item->id}}@endslot--}}
            @slot('title')
                <h1>Редактирование заказ № {{$item->id}}</h1>
                @if(!$order->status)
                ...
                @else
                    <a href="{{route('blog.admin.orders.change', $item->id)}}/?status=0" class="btn btn-success btn-xs">Вернуть на доработку</a>
                @endif
                <a href="#" class="btn btn-xs">
                    <form action="{{route('blog/admin.orders.destroy', $item->id)}}" id="deleteForm" method="post">
                        @method('DELETE')
                        @csrf
